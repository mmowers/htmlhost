 ...
 */
function sub_seven_preprocess_maintenance_page(&$vars) {
 ...
 */
function sub_seven_preprocess_html(&$vars) {
  if($node = menu_get_object()){
    if($node->type == "page" && !preg_match('/edit\/?$/', current_path())){
      $vars['theme_hook_suggestions'][] = 'html__custompage';
      $vars['safe_html'] = $node->field_html['und'][0]['safe_value'];
    }
  }
 ...
 */
function sub_seven_preprocess_page(&$vars) {
 ...
 */
function sub_seven_node_add_list($variables) {
 ...
 */
function sub_seven_admin_block_content($variables) {
  $content = $variables['content'];
  $output = '';
  if (!empty($content)) {
 ...
 */
function sub_seven_tablesort_indicator($variables) {
 ...
 */
function sub_seven_css_alter(&$css) {
